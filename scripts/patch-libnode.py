#!/usr/bin/env python3
# patch-libnode.py
# Patch EnableTrapHandler dans libnode.so pour retourner false immediatement.
#
...
# juste __builtin_trap() -> BRK -> SIGTRAP -> crash fatal.
# Fix : remplacer les 2 premieres instructions par MOV X0,XZR ; RET
# La fonction retourne false immediatement, sans crash.
...
def main():
    ...
    if i0 == MOV_X0_ZERO and i1 == RET:
        print("[patch-libnode] Deja patche, rien a faire.")
        return
    ...
    struct.pack_into("<I", data, file_off, MOV_X0_ZERO)
    ...
    assert v0 == MOV_X0_ZERO and v1 == RET, "Verification echouee: {} {}".format(hex(v0), hex(v1))
    print("[patch-libnode] OK: EnableTrapHandler retourne false immediatement.")
